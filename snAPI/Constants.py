...
class DeviceType(Enum):
    """
The MultiHarp devices can use their internal crystal clock but alternatively they can be synchronized
...
an experiment must be avoided. The constants defined here specify the reference clock to use.
    ...
    File = 1
    """
    This is a file device. That means a ptu File that is opened as a device.
...
| This is the default and normally what most users would select when only one device is being used. 
    ...
    **10MHz external clock**
    This is for use with an industry standard 10MHz reference such as an atomic clock or another MultiHarp.
